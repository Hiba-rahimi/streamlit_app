...
uploaded_recycled_file = st.file_uploader(":arrow_down: **Upload Recycled Transactions File**", type=["csv"])
st.divider()
...
date_ref = None  # Initialize date_ref variable

if uploaded_mastercard_file is not None:
    
# extracting date from mastercard to be used for validiting the date of the other sources
    try: 
        date_ref = extract_date_from_mastercard_file(uploaded_mastercard_file.getvalue().decode("utf-8"))
        st.write("**You will be performing reconciliation for date :calendar:**", date_ref)
...
try:
    if uploaded_cybersource_file:
        ...
        validate_file_name_and_date(uploaded_cybersource_file.name, 'CYBERSOURCE', date_to_validate=date_ref)
...
try:
    if uploaded_pos_file:
        ...
        validate_file_name_and_date(uploaded_pos_file.name, 'POS', date_to_validate=date_ref)
    ...
    st.error(f"Error processing POS file check your uploaded file")

def get_image_download_link(img_data, filename, text):
    """
    Generates a download link for the image.
    """
    b64_img = base64.b64encode(img_data.getvalue()).decode()
    href = f'<a href="data:image/png;base64,{b64_img}" download="{filename}">{text}</a>'
    return href

try:
    if uploaded_sai_manuelle_file:
        sai_manuelle_file_path = save_uploaded_file(uploaded_sai_manuelle_file)
        validate_file_name_and_date(uploaded_sai_manuelle_file.name, 'SAIS_MANU', date_to_validate=date_ref)
...
try:
    if uploaded_mastercard_file:
        ...
        if uploaded_recycled_file:
            ...
            merged_df, total_nbre_transactions = merging_with_recycled(recycled_file_path, filtered_cybersource_df, filtered_saisie_manuelle_df, filtered_pos_df)
